...
def getduration(origin_path, result_path, MyAK, mytime):
    url_drive = "http://api.map.baidu.com/directionlite/v1/driving?origin=" + str(origin_path[0]) + "," + str(
        origin_path[1]) + "&destination=" + str(result_path[0]) + "," + str(result_path[1]) + "&tactics=3&ak=" + MyAK
    url_walk = "http://api.map.baidu.com/directionlite/v1/walking?origin=" + str(origin_path[0]) + "," + str(
        origin_path[1]) + "&destination=" + str(result_path[0]) + "," + str(result_path[1]) + "&tactics=3&ak=" + MyAK
    url_ride = "http://api.map.baidu.com/directionlite/v1/riding?origin=" + str(origin_path[0]) + "," + str(
        origin_path[1]) + "&destination=" + str(result_path[0]) + "," + str(result_path[1]) + "&tactics=3&ak=" + MyAK
    url_bus = "http://api.map.baidu.com/directionlite/v1/transit?origin=" + str(origin_path[0]) + "," + str(
        origin_path[1]) + "&destination=" + str(result_path[0]) + "," + str(result_path[1]) + "&tactics=3&ak=" + MyAK
    print(url_drive)
    print(url_walk)
    print(url_ride)
    ...
    if len(answer0['result']['routes']) == 0:
        ...
        distance[0] = answer0['result']['routes'][0]['distance']
    # 从API读取步行数据
    ...
    if len(answer1['result']['routes']) == 0:
        ...
        distance[1] = answer1['result']['routes'][0]['distance']
    # 从API读取骑行数据
    ...
    if len(answer2['result']['routes']) == 0:
        ...
        distance[2] = answer2['result']['routes'][0]['distance']
    # 从API读取公交数据
    ...
    if answer3['message'] != "ok":
        ...
        distance[3] = answer3['result']['routes'][0]['distance']
        steps = answer3['result']['routes'][0]['steps']
        print(steps[0][0]['type'])
    print(answer)
    print(mytime)
    print(distance)
    ...
    #             print(steps[i][0]['type'])
    #             list.append(steps[i][0]['type'])
    type1 = ''
    z = ''
    if y == 4:
        l1 = len(answer3['result']['routes'][0]['steps'])
        for i in range(0, l1):
            type2 = str(answer3['result']['routes'][0]['steps'][i][0]['type'])
            print(answer3['result']['routes'][0]['steps'][i][0]['type'])
            type1 += type2
            type1 += ','
        if (type1.find('1') != -1) | (type1.find('12') != -1) | (type1.find('13') != -1):
            z = 'YES'
        else:
            z = 'NO'
    print(type1)
    print(y)
    print(distance[y - 1])
    return y, distance[y - 1], answer[y - 1], type1, z
...
for item in df_travel.iterrows():
    ...
    df_travel.loc[[item[0]], 'type'] = res[0]
    df_travel.loc[[item[0]], 'distance'] = res[1]
    df_travel.loc[[item[0]], 'duration'] = res[2]
    df_travel.loc[[item[0]], 'bustype'] = res[3]
    df_travel.loc[[item[0]], 'hasSubway'] = res[4]
...
# del df_1['startstamp']
# del df_1['endstamp']
df_1['speed'] = df_1['distance'] / df_1['duration']
df_1['real_speed'] = df_1['distance'] / df_1['mytime']
# df_1.to_csv('Result.csv')

df_1.sort_values('type', inplace=False)
df_1

df_1.to_csv('newTripModeResult.csv', index=None)
